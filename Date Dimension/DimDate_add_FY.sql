...
DECLARE
	@dtFiscalYearStart SMALLDATETIME = 'July 01, 2010',
	@FiscalYear INT = 2010,
	@LastYear INT = 2025,
...
WHEN @FiscalWeekOfYear BETWEEN (48+@leapWeek) AND (52+@leapWeek) THEN 12 /*5 weeks*/
    ...
    SELECT @FiscalYearName = 'FY ' + CONVERT(VARCHAR, @FiscalYear+1)

    INSERT INTO @tb
        (PeriodDate, FiscalDayOfYear, FiscalWeekOfYear,
        fiscalMonth, FiscalQuarter, FiscalQuarterName, FiscalYear, FiscalYearName)
    VALUES
...
SET
	FiscalMonthYear = 
		...
		WHEN 11 THEN 'Nov'
		WHEN 12 THEN 'Dec'
		END + '-' + CONVERT(VARCHAR, FiscalYear+1)
